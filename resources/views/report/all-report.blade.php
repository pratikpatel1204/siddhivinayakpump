...
@section('title', config('app.name') . ' || All Report')
@section('content')
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="gift_table">
                                ...
                                <tbody>
                                    @foreach ($giftHistory as $ghistory)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $ghistory->type }}</td>
                                        <td>{{ $ghistory->name }}</td>
                                        <td>{{ $ghistory->village_city }}</td>
                                        <td>{{ $ghistory->district }}</td>
                                        <td>{{ $ghistory->vehicle_no }}</td>
                                        <td>{{ $ghistory->mobile_no }}</td>
                                        <td>{{ $ghistory->used_reward_points }}</td>
                                        <td>{{ $ghistory->emp->name }}</td>
                                        <td>{{ $ghistory->updated_at }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
...
<script>
    $(document).ready(function() {
        ...
        function fetchFilteredData() {
            ...
            $.ajax({
                url: "{{ route('all.report.reward.filter') }}"
                , type: 'GET'
                , data: {
                    date: selectedDate
...
<script>
    $(document).ready(function() {
        ...
        function fetchFilteredgift() {
            ...
            $.ajax({
                url: "{{ route('all.report.gift.filter') }}"
                , type: 'GET'
                , data: {
                    date: selectedDateg
                    , employee: selectedemployee
